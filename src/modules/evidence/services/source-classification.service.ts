...
export type SourceType = 'GOVERNMENT' | 'NEWS' | 'RESEARCH_PAPER' | 'BLOG' | 'SOCIAL_MEDIA' | 'INTERNAL_REPORT';
...
const classifiers: SourceClassifier[] = [
  // Government domains
 ...
 */
export function classifySource(sourceUrl: string): SourceType {
  try {
 ...
 */
export function getSourceCredibilityTier(sourceType: SourceType): number {
  const tiers: Record<SourceType, number> = {
    GOVERNMENT: 0.95,
    RESEARCH_PAPER: 0.85,
    NEWS: 0.7,
    BLOG: 0.5,
