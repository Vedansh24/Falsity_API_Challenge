...
export async function fetchWikipediaSummary(query: string) {
  ...
  const sres = await fetchWithRetry(searchUrl, { method: 'GET' }, { timeoutMs: 6000, retries: 1 });
  if (!sres.ok) return sres;

  const title = sres.data?.[1]?.[0];
  if (!title) return { ok: false, error: { code: 'NOT_FOUND', message: 'No Wikipedia page found' } };

  const summaryUrl = `${WIKI_API}/page/summary/${encodeURIComponent(title)}`;
  const res = await fetchWithRetry(summaryUrl, { method: 'GET', headers: { 'User-Agent': 'falsity-api/1.0' } }, { timeoutMs: 6000, retries: 1 });
