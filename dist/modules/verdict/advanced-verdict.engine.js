...
exports.computeWeightedScores = computeWeightedScores;
exports.analyzeContradictions = analyzeContradictions;
exports.calculateFalsityScore = calculateFalsityScore;
exports.computeAdvancedVerdict = computeAdvancedVerdict;
// ============================================================================
 ...
 */
function calculateEvidenceWeight(evidence) {
 ...
 */
function computeWeightedScores(evidences) {
 ...
 */
function analyzeContradictions(evidences, supportScore, contradictScore) {
 ...
 */
function applyContradictionPenalty(baseConfidence, contradictionLevel) {
 ...
 */
function calculateFalsityScore(supportScore, contradictScore) {
    const totalScore = supportScore + contradictScore;
 ...
 * - Contradiction level
 */
function calculateConfidenceScore(supportScore, contradictScore, evidenceCount, averageCredibility, contradictionLevel) {
    ...
    // Combine all factors
    let confidence = (baseStrengthConfidence * 0.4 +
        countConfidence * 0.3 +
 ...
 */
function getConfidenceBand(confidenceScore) {
 ...
 */
function buildAdvancedReasoning(evidences, context, contradictionAnalysis, falsityScore) {
    const { supportScore, contradictScore, verdict, contradictionLevel, averageCredibility } = context;
    ...
    parts.push(`Analyzed ${total} piece${total !== 1 ? 's' : ''} of evidence: ` +
    ...
    if (supportScore > contradictScore) {
        ...
        parts.push(`Supporting evidence dominates with ${ratio}% weighted strength. ` +
        ...
        parts.push(`Contradicting evidence dominates with ${ratio}% weighted strength. ` +
            `Average credibility of sources: ${(averageCredibility * 100).toFixed(0)}%.`);
    }
    else {
        parts.push(`Support and contradict evidence have equal weighted strength. ` +
    ...
    if (contradictionAnalysis.hasSupportingEvidence && contradictionAnalysis.hasContradictingEvidence) {
        if (contradictionLevel > 0.7) {
            parts.push('⚠️ SIGNIFICANT CONTRADICTION: Both strong supporting and contradicting ' +
                'evidence exist, reducing confidence in this verdict.');
        }
        else if (contradictionLevel > 0.4) {
            parts.push('Note: Some contradictory evidence exists, moderately affecting confidence.');
        }
    }
 ...
 */
function computeAdvancedVerdict(evidences) {
    ...
    const contradictionAnalysis = analyzeContradictions(evidences, supportScore, contradictScore);
    ...
    const baseConfidence = calculateConfidenceScore(supportScore, contradictScore, evidences.length, averageCredibility, contradictionAnalysis.contradictionLevel);
    ...
    const reasoning = buildAdvancedReasoning(evidences, context, contradictionAnalysis, falsityScore);
