  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Operational alerts for assignment changes, evidence requests, and reviewer handoff readiness."
      />

      <Card>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm text-neutral-600">{items.length} notifications</div>
          <Button variant="outline" onClick={() => markAllRead.mutate(undefined)} disabled={markAllRead.isLoading}>
      ...
      <Card>
        <div className="space-y-3">
          ...
          {!notificationsQuery.isLoading && items.length === 0 && <div className="text-sm text-neutral-600">No new analyst updates.</div>}

          {!notificationsQuery.isLoading && items.map((item: unknown, index: number) => {
            ...
            const investigationId = toStringValue(record.investigationId || metadata.investigationId);
            ...
            return (
              <div key={toStringValue(record.id) || String(index)} className="rounded-lg border border-neutral-200 p-3">
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  {investigationId ? (
                    <Link href={`/dashboard/analyst/investigations/${investigationId}`} className="text-xs underline">
                      Open investigation
                    </Link>
                  ) : null}
                </div>
